...
extension AppModel {
    var endpointCheckSteps: [EndpointCheckStep] {
        ...
        let providerStatus = latestCommandStatus(.providerUpdate, in: session)
        ...
        return [
            ...
            EndpointCheckStep(
                ...
                detail: isSurge ? localized("trial.surge_provider_not_included") : (providersCapability == .supported ? lastResultLabel(for: .providerUpdate, in: session) : localized("trial.provider_unavailable")),
    ...
    var checkResultRows: [CheckResultRow] {
        ...
        return [
            ...
            CheckResultRow(
                ...
                latestResult: lastResultLabel(for: .providerUpdate, in: session),
                lastChecked: lastCheckedLabel(for: .providerUpdate, in: session),
    ...
    private func lastResultLabel(for action: TrialCommandAction, in session: TrialSessionSnapshot) -> String {
        guard let entry = latestCommandEntry(action, in: session) else {
    ...
    private func lastCheckedLabel(for action: TrialCommandAction, in session: TrialSessionSnapshot) -> String {
        safeTimeLabel(latestCommandEntry(action, in: session)?.timestamp)
    }
