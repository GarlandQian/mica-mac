...
extension AppModel {
    ...
    func removeSessionConnections(_ connections: [ConnectionSnapshot]) {
        ...
        if let router = selectedRouter {
            ...
            case .singBoxCompatible:
                controllerSession.singBoxActiveConnections.removeAll {
                    closedIDs.contains($0.id)
                }

            case .surgeCompatible:
                let projectedIDs = DashboardSnapshot.surgeRequestDisplayIDs(
                    for: controllerSession.surgeRawSnapshot.activeRequests
                )
                controllerSession.surgeRawSnapshot.activeRequests = zip(
                    controllerSession.surgeRawSnapshot.activeRequests,
                    projectedIDs
                ).compactMap { request, projectedID in
                    closedIDs.contains(projectedID) ? nil : request
                }

            default:
                if let response = controllerSession.endpointCache.connections {
                    controllerSession.endpointCache.connections = ConnectionsResponse(
        ...
        mutateSessionDashboard(publishing: [.connections, .insight]) { dashboard in
    ...
    func mutateSessionDashboard(
        publishing domains: DashboardPublicationDomains,
