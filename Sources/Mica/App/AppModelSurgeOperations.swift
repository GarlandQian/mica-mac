...
extension AppModel {
    ...
    func setSurgeOutboundMode(_ mode: String) {
        ...
        guard controllerSupports(.setOutboundMode, router: router, action: TrialCommandAction.surgeOutboundMode.title(language: presentationLanguage)) else {
            return
        }
    ...
    func selectSurgePolicy(_ policy: String, in group: String) {
        ...
        guard controllerSupports(.selectSurgePolicy, router: router, action: TrialCommandAction.surgePolicySelect.title(language: presentationLanguage)) else {
            return
        }
    ...
    func testSurgePolicyGroup(_ group: String) {
        ...
        guard controllerSupports(.testSurgePolicy, router: router, action: TrialCommandAction.surgePolicyTest.title(language: presentationLanguage)) else {
            return
        }
    ...
    func killSurgeRequest(
        ...
        guard controllerSupports(.killActiveRequest, router: router, action: TrialCommandAction.surgeRequestKill.title(language: presentationLanguage)) else {
            return
        }
    ...
    func applySurgeSnapshot(
        ...
        var projectedDashboard = projectedSurgeDashboard(
            snapshot,
            connectionRatesReceivedAt: connectionRatesReceivedAt
        )
        projectedDashboard.controllerLogs = sessionActionDashboard.controllerLogs

        let nextUnified = UnifiedControllerSnapshot.surgeHTTPAPI(profile: router, snapshot: snapshot)
        let nextProvidersState = EnhancedSnapshotState.unavailable(
            localized("trial.surge_provider_not_included")
        )
        let nextHealth = Self.surgeHealth(
            router: router,
            snapshot: snapshot,
        ...
        if dashboardSessionControls.dashboardUpdatesPaused {
            ...
            controllerSession.pendingPresentation.dashboard = projectedDashboard
            controllerSession.pendingPresentation.unifiedSnapshot = nextUnified
            controllerSession.pendingPresentation.rulesSnapshotState = .available
            controllerSession.pendingPresentation.providersSnapshotState = nextProvidersState
            controllerSession.pendingPresentation.controllerHealth = nextHealth
            return
        }
    ...
    func closeAllSurgeProjectedConnections(router: RouterProfile) {
        ...
        let projectedIDs = DashboardSnapshot.surgeRequestDisplayIDs(for: surgeSnapshot.activeRequests)
        let requests = Array(zip(surgeSnapshot.activeRequests, projectedIDs))
        let commandID = beginCommand(
            .closeAll,
            router: router,
    ...
    private func surgeRequest(forProjectedConnectionID projectedID: String) -> SurgeActiveRequest? {
        let projectedIDs = DashboardSnapshot.surgeRequestDisplayIDs(for: surgeSnapshot.activeRequests)
        return zip(surgeSnapshot.activeRequests, projectedIDs).first { _, id in
            id == projectedID
        }?.0
    }
}
