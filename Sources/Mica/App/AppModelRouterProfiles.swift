...
extension AppModel {
    ...
    func deleteRouterTransaction(_ router: RouterProfile) async throws {
        ...
        do {
            ...
            removePolicyGroupPresentationState(for: router.id)
    ...
    func testConnection(draft: RouterDraft) async -> ConnectionTestReport {
        ...
        if resolvedKind == .surgeCompatible {
            let client = SurgeHttpAPIClient(profile: profile, apiKey: credential)
            ...
            do {
                ...
                finishCommand(commandID, routerID: profile.id, status: .failed, summary: localized("operation.surge_profile_test_failed"))
                return ConnectionTestReport.failure(draft: draft, error: error, language: presentationLanguage)
            }
        }
