...
const fontScaleExpectations = {
  standard: { dynamicTypeSize: "small", multiplier: 0.92, calloutPointSize: 11 },
  comfortable: { dynamicTypeSize: "large", multiplier: 1, calloutPointSize: 12 },
  large: { dynamicTypeSize: "xxLarge", multiplier: 1.16, calloutPointSize: 14 },
  extraLarge: { dynamicTypeSize: "xxxLarge", multiplier: 1.32, calloutPointSize: 16 },
};
...
function assertFontScale(actual, rawValue, context) {
  assertFields(actual, { rawValue, ...fontScaleExpectations[rawValue] }, context);
}
...
function assertSystemMenuProjection(payload, context) {
  const title = payload.menuSamples?.["controller.command_menu"];
  assert(
    title === "Controller" || title === "控制器",
    `${context}: custom top-level menu did not resolve through the supported system language`
  );

  for (const [language, transition] of Object.entries(payload.preferenceTransitions?.languages ?? {})) {
    assert(
      transition.menuSamples?.["controller.command_menu"] === title,
      `${context}: ${language} transition changed the system-language top-level menu`
    );
  }
}
...
function assertSmokePayload(payload, testCase) {
  ...
  assertSystemMenuProjection(payload, context);
