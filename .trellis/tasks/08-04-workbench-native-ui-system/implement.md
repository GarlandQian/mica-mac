...
- [ ] 完成并验收 `08-03-connections-native-visual-rebuild` 的剩余 Connections/Proxies 工作。
- [ ] 运行该子任务自己的定向投影、排序、关闭安全、本地化和 build/test 质量门。
- [ ] 更新相关 Trellis spec、提交并归档子任务。
- [ ] 在父任务记录实际完成范围和任何共享视觉原语变化。
...
- [ ] 盘点全部共享 token/primitive 消费者，删除重复或失效视觉路径，但不创建平行 design system。
- [ ] 统一 Window/Toolbar/Command Bar/Content/Status 的 page fill 和 separator。
- [ ] 保持 10 个目的地，修复侧边栏整行点击、选中态、字号和键盘导航。
- [ ] 保持原生 Settings Scene；只渲染语言、外观、四档字号和 GLOBAL 可见性。
- [ ] 验证设置在主窗口与 Settings 窗口即时生效，且字号不改变页面断点。
- [ ] 确认 Command Palette/Deck/Cmd+K 路径不存在，toolbar 不重复 controller identity。
...
- [ ] 审核现有 personalization/runtime/projection，保留真实模块和每窗口事务草稿。
- [ ] 重构 telemetry 为上传、下载、连接三块同权图表；内存作为连接上下文，不恢复第四张图。
- [ ] 完整保留 Chart hover、pin、step、pause、return-live 和时间窗口交互。
- [ ] 保留完整 Sankey 拓扑、网络信息、无横向滚动和同窗口展开/连接页导航。
- [ ] 缺失能力或无数据时隐藏模块或显示准确状态，不制造样本。
- [ ] 窄/中/宽布局只按可用宽度变化，个性化顺序和可见性保持稳定。
...
- [ ] 提取 Settings、Controllers、Configuration、Actions、Diagnostics 到职责文件，保留现有类型和访问级别。
- [ ] 机械移动后先运行一次 Swift build，确认没有行为变化，再进入 UI 修改。

### Controllers and RouterEditor

- [ ] Controllers 使用稳定 master-detail，编辑/删除在右侧上下文，只有 Use 切换活动控制器。
- [ ] 保持用户配置顺序和显式移动操作；不按健康、名称或延迟自动重排。
- [ ] RouterEditor 保持同窗口覆盖、原生 Form、一次性提交和现有 FileSecretStore 路径。

### Configuration and Actions

- [ ] Configuration 只显示 controller metadata/config 中真实报告且支持修改的字段。
- [ ] Actions 只显示现有 supported operation；不支持项不形成噪音列表，破坏性操作行内确认。
- [ ] 所有 mutation 和 confirmation 校验 controller ID、generation 和目标。

### Diagnostics

- [ ] 顶部改为人类可读的紧凑结论带；详情使用一个同窗口 outline。
- [ ] 一级/二级 disclosure 整行可点、动画一致；展开后滚动不因嵌套 lazy 容器卡顿。
- [ ] 隐藏 unavailable capability 行和空 section；页面不显示 API 路径、机器 assignment 或 raw payload。
- [ ] Copy Report 保留凭据安全的技术详情。
...
- [ ] 用户审核并明确批准本 PRD、设计和分阶段计划。
- [ ] 父任务通过 `task.py validate`。
- [ ] 当前 Connections/Proxies 子任务的实际状态已确认，不重复安排已完成工作。
- [ ] Phase 1 子任务创建后才运行 `task.py start`；本次规划消息不修改应用源码。
