...
Auto Detect resolves the HTTP family first. A concrete HTTP identity starts immediately, while an unrecognized or ambiguous HTTP result falls through to the StartedService version probe. A conclusive refused, missing, or unavailable port enters bounded retry without waiting for the gRPC readiness timeout; authentication and terminal target errors are never silently reclassified.
