...
- The policy scroll owner contains one root adaptive `LazyVGrid`. Groups are direct source-ordered `Section` values whose headers own summary, commands, latency distribution, and expanded filter; node tiles are direct section content. Never restore a same-axis `LazyVStack -> group panel -> LazyVGrid` nesting, because it virtualizes giant variable-height groups rather than visible node tiles.
...
- The policy page projects `PolicyGroupCatalogSnapshot`, which changes only with mode or group data. A zero-size nonvisual leaf observes selected controller ID, presentation generation, and scalar catalog revision, captures the matching complete snapshot, and submits it through the existing latest-wins scroll scheduler. The expanded visual root consumes only accepted page state and never directly observes the complete catalog. High-frequency traffic, connection, provider, rule, and log updates must not force policy headers or expanded-group projections to recompute during scrolling.
