...
Wide telemetry uses three chart columns, medium telemetry uses two traffic columns plus a full connection row, and narrow telemetry stacks visible metrics. Plot height follows effective panel width within a stable 240-to-300-point range, while sparse topology keeps a width-responsive 680-to-920-point minimum flow area and dense topology may grow further. Overview section titles use consistent informational-cyan 34-point marks and metric titles use 28-point marks tied to real series meaning, both with hierarchical native SF Symbols; category icons never borrow mint healthy or violet debug roles, and dense rows retain unbacked monochrome symbols. Every chart shares real timestamps, hover/single-click pin selection, keyboard stepping, pause, and return-to-live; hover updates only readouts and indicators, while only a pinned sample changes the header to Selected. Memory is contextual metadata on the connection chart rather than a fourth plot. Zero-value and one-sample timelines remain visible through a presentation-only padded scale and a latest-real-sample point; this never fabricates samples. Overview does not contain retained closed data, a raw connection selector, or the full connection inspector.

The topology uses 20-point node bars with 8-point gaps and complete callout labels. Hovering a policy node opens a complete node-anchored field HUD; clicking pins the same field composition, transient hover temporarily overrides a pin and restores it on exit, and Escape or blank-canvas activation clears selection. The HUD reads only the current published policy catalog, uses exact case-sensitive unique matching, exposes ordered members, typed node/test/transport fields, and every additional controller field in stable key order, and performs no network request. Duplicate or missing names fall back to truthful route detail. Candidate placement scores trailing, leading, above, and below positions against graph bounds and node/label obstacles, then clamps inside the graph without moving topology or scroll geometry. Eligible Proxies and Connections navigation remains in the same window, and VoiceOver exposes policy-node values and pinned state.
