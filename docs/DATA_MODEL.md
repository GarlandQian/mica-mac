...
- Overview: selected controller identity and session health; four current live readouts; generation-scoped real traffic, memory, and active-connection timelines; short ordered latency, rule-hit, and active-connection summaries; compact controller/network facts; and a complete topology containing every active connection plus every reported route-chain hop. Missing route metadata remains an explicit unavailable path without fake edges. Retained closed history and full connection fields belong to Connections.
