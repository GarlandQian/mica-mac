...
final class ControllerHTTPProbeResolverTests: XCTestCase {
    ...
    func testConnectionRefusedDoesNotStartSurgeProbe() async throws {
        let fixture = ControllerProbeFixture(defaultError: URLError(.cannotConnectToHost))
        ...
        do {
            ...
            XCTFail("Expected connection refusal")
...
private actor ControllerProbeFixture {
    ...
    private let defaultError: URLError?
    private var requests: [URLRequest] = []

    init(
        routes: [String: Route] = [:],
        defaultRoute: Route = .status(404),
        defaultError: URLError? = nil
    ) {
        self.routes = routes
    ...
    func load(_ request: URLRequest) throws -> (Data, URLResponse) {
        ...
        if let defaultError, routes[request.url?.path ?? ""] == nil {
            throw defaultError
        }
